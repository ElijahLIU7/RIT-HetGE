...
def train_hetsann_model(train_dataset, valid_dataset, embed_size, epochs=50, lr=0.001, batch_size=8, fold=None, weight_decay=0.0005):
    ...
    # Optimizer
    optimizer = optim.Adam(hetsann.parameters(), lr=lr, weight_decay=weight_decay)
    ...
    for epoch in range(epochs):
        ...
        val_loss = regression_loss(th.tensor(all_preds), th.tensor(all_labels).float())  # Validation set loss
        # 计算 MSE
    ...
    return hetsann, best_loss, mse, r2, mae, pcc, best_model_path
...
def cross_validation_hetsann(dataset_path, CV_FOLDS=10, epochs=50, lr=0.1, batch_size=8, hidden_feats=512, weight_decay=0.0005, bidirected=False):
    all_losses = []
    feat_dim = None
    relations = None
    ...
    for cv_select in range(CV_FOLDS):
        ...
        hetsann_model, best_loss, mse, r2, mae, pcc, model_path = train_hetsann_model(train_dataset, valid_dataset,
...
if '__main__' == __name__:
    ...
    all_mae = []
    all_pcc = []
    # best_model_path, _ = cross_validation_hetsann(dataset_path, CV_FOLDS=CV_FOLDS, epochs=200, lr=0.0001, batch_size=8,
    #                                       hidden_feats=hidden_feats, bidirected=False, weight_decay=0.0005)
    ...
    for fold in range(CV_FOLDS):
        ...
        mse, r2, mae, pcc = test_best_hetsann_model(test_dataset, embed_size=feat_dim, model_path=best_model_path, batch_size=8)
        # 保存指标
        all_mse.append(mse)
        all_r2.append(r2)
        all_mae.append(mae)

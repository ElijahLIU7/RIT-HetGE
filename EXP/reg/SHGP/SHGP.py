...
def train_att_hgcn_model(train_dataset, valid_dataset, net_schema, embed_size,
                         epochs=50, lr=1e-4, batch_size=8, type_fusion='att', type_att_size=64, fold=None):
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch + 1}/{epochs}") as pbar:
            for i, batch in enumerate(pbar):
                ...
                # adj_dict = {j: {k: v.to(device) for k, v in adj_dict.items()} for j, _ in ft_dict.items()}
...
def test_best_model(test_dataset, hidden_feats, model_path, batch_size=8):
    """使用测试集评估最优模型"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    layer_shape = [572, hidden_feats, hidden_feats]
    net_schema = {'node': ['VDW', 'PIPISTACK', 'HBOND', 'IONIC', 'SSBOND', 'PICATION']}     # ['VDW', 'PIPISTACK', 'HBOND', 'IONIC', 'SSBOND', 'PICATION']
...
def cross_validation(dataset_path, CV_FOLDS=10, epochs=50, lr=0.001, batch_size=8, hidden_feats=128):
    ...
    for cv_select in range(CV_FOLDS):
        ...
        dhne_model, best_loss, mse, r2, mae, pcc, model_path = train_att_hgcn_model(train_dataset, valid_dataset, net_schema=net_schema,
...
def objective(trial):
    ...
    for fold in range(CV_FOLDS):
        ...
        mse, r2, mae, pcc = test_best_model(test_dataset, hidden_feats=hidden_feats, model_path=best_model_path, batch_size=8)
        # 保存指标
        all_mse.append(mse)
        all_r2.append(r2)
        all_mae.append(mae)

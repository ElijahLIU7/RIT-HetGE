...
class GCN(nn.Module):
    def __init__(self, in_feats, hidden_feats, out_feats, num_layers=4, num_heads=8):
        ...
        # 输入层，使用多头注意力
        self.layers.append(SAGEConv(in_feats, hidden_feats, aggregator_type='pool', feat_drop=0.1))

        # 隐藏层，每层使用多头注意力
        for _ in range(num_layers - 2):
            self.layers.append(SAGEConv(hidden_feats, hidden_feats, aggregator_type='pool', feat_drop=0.1))

        # 输出层
        self.layers.append(SAGEConv(hidden_feats, out_feats, aggregator_type='pool', feat_drop=0.1))
    ...
    def forward(self, g, x):
        for i, conv in enumerate(self.layers):
            ...
            if i != len(self.layers) - 1:  # 最后一层不需要再拼接
                x = x.view(x.size(0), -1)
        g.ndata['h'] = x
        x_squeezed = x.squeeze()  # 确保 x 是 1D Tensor
        x_len = x_squeezed.shape[0]  # 获取当前长度
        if x_len < 30:
            # 在末尾补零，使其达到 30
            x_padded = F.pad(x_squeezed, (0, 30 - x_len), "constant", 0)
        else:
            x_padded = x_squeezed[:30]
        return dgl.mean_nodes(g, 'h').squeeze(), x_padded.reshape(1, -1)  # 图级表示，用于分类
...
def test_best_model(test_dataset, feat_dim, hidden_feats, num_classes, model_path, batch_size=8):
    ...
    with torch.no_grad():
        for batch in test_loader:
            graphs, labels = batch[0].to(device), batch[1].to(device)
            features = graphs.ndata['emb'].to(device)
            logits, layer_features = model(graphs, features)
            ...
            all_labels.extend(labels.cpu().numpy())
            all_preds.extend([predicted.cpu().item()])
            all_probs.extend([probs.cpu().item()])
    # 转换为numpy数组
...
def plot_tsne(embeddings, true_labels, pred_labels):
    ...
    sns.scatterplot(
        ...
        hue='Predicted Label', palette=binary_palette,
        data=df,
        legend='full',
        alpha=0.7
    )
    plt.title('t-SNE Visualization of GraphSAGE')

    plt.tight_layout()
    plt.savefig("t-SNE_GCN.png", dpi=300)
    plt.show()
...
for fold in range(CV_FOLDS):
    ...
    auc, f1, precision, recall, accuracy = test_best_model(test_dataset, feat_dim, hidden_feats=hidden_feats, num_classes=1, model_path=best_model_path, batch_size=1)
    # 保存指标
    all_aucs.append(auc)
    all_f1s.append(f1)

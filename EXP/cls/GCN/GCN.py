...
class GCN(nn.Module):
    def __init__(self, in_feats, hidden_feats, out_feats, num_layers=4):
        ...
        # 输入层
        self.layers.append(GraphConv(in_feats, hidden_feats, activation=F.relu, allow_zero_in_degree=True))

        # 隐藏层
        for _ in range(num_layers - 2):
            self.layers.append(GraphConv(hidden_feats, hidden_feats, activation=F.relu, allow_zero_in_degree=True))
...
def test_best_model(test_dataset, feat_dim, hidden_feats, num_classes, model_path, batch_size=1):
    """使用测试集评估最优模型"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = GCN(in_feats=feat_dim, hidden_feats=hidden_feats, out_feats=num_classes).to(device)
